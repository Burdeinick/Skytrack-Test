...
Скачать проект из GitHub любым известным способом, войти в дирректорию `Skytrack-Test`. Все команды выполнять из данной директории.
...
Так как данное API является тестовым и в задании не было строго указания по каким полям таблиц осуществлять запросы информации, по этой причине практически везде были выбраны 'id' таблиц.

Так же реализовано удаление всех таблиц БД, если это необходимо сделать. Для этого необходимо ввести в bash: `$ make droptables`.
